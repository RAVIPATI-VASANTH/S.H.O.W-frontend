...
function PlayGround() {
  ...
  const [sendToNextPlayerButtonSignal, setSendToNextPlayerButtonSignal] =
    useState(false);
  const [declareShowButtonSignal, setDeclareShowButtonSignal] = useState(false);
  const loadGameData = () => {
  ...
  const declareShow = () => {
    setDeclareShowButtonSignal(true);
    setIsCardSelected(false);
    let localRoom = {
      roomCode: localStorage.getItem("roomCode"),
      cardsCount: Number(localStorage.getItem("cardsCount")),
    ...
    fetch(`${constants.backendUrl}/get-error-reponse`, {
      ...
      let status = res.status;
      res.json().then((res) => {
        setDeclareShowButtonSignal(false);
        switch (status) {
          case 200:
            setResponse(res.message);
            setTimeout(() => {
          ...
          {loaderSignal ? (
            ...
            <div className="flex flex-col gap-2 p-2 items-center justify-center">
              ...
              <button
                ...
                onClick={declareShow}
                disabled={declareShowButtonSignal}
              >
